...
2026-09-07 Phase B — Standard Workflow 코드 SoT 상수화(비파괴): `lib/deal/standard-workflow.ts`에 Seller/Buyer 13단계를 타입 안전 상수(`key·side·order·label·description·phase·optional·stage`)로 정의하고 `WorkflowSide/WorkflowPhase(PREPARE·ENGAGE·EVALUATE·EXECUTE·INTEGRATE)/WorkflowStageKey/StandardWorkflowStage` 타입 + `getStandardWorkflow`/`getStandardStageByKey`를 제공한다. `lib/landing/ma-workflow.ts`는 이 SoT에서 label/desc/key를 파생한다(문구·순서·UI 결과 불변, 중복 제거). Mandate만 `optional:true`(위치 유지). Enum/DB/persistence/TOM/Workspace/route/UI 동작 미변경.

2026-09-07 Phase C — Seller Standard Stage Persistence Foundation(코드+migration 준비, Production 미적용): Deal = Seller 매각 프로젝트, Opportunity = Seller↔Buyer 1:1 path 원칙에 따라 **Seller 현재 단계만** `deals.seller_stage_key`(nullable, default 없음, 값=코드 SoT `SELLER_*` key)에 저장한다. **Buyer stage는 deals에 넣지 않는다**(→ Opportunity 기반 후속 Phase). 전환 이력은 append-only `deal_stage_events`(deal-scoped RLS)로 남기고, 일반 감사 로그는 기존 `recordAudit()`(`audit_logs`/`activities`)로 병행한다(기존 audit은 actor-scoped라 from/to stage 이력에 부적합해 도메인 테이블을 별도 추가). 유효 stage key(13개) 검증은 서버 코드 SoT(`isValidSellerStageKey`)가 primary, DB는 `SELLER_` prefix 형태 CHECK만(정확 열거/PostgreSQL enum 미사용 → code-DB drift 최소화·확장/override 대비). 단일 write path는 security-definer RPC `transition_seller_deal_stage`(권한 검증+event append+현재 stage 갱신 원자적) + 서버 boundary `transitionSellerDealStage`; 직접 INSERT/UPDATE 권한은 authenticated에 부여하지 않는다. 쓰기 권한 Role = SELLER_OWNER/SELLER_OPERATOR/INTERNAL_MANAGER(Advisor 이번 제외). 자동 진행 trigger·순서 강제(Mandate 필수)·AI Stage 확정 없음. 기존 Deal은 `seller_stage_key=NULL` 유지(자동 DISCOVERY 금지). 파일: `supabase/migrations/0018_seller_standard_stage.sql`(additive, **원격 미적용**), `lib/deal/seller-stage-transition.ts`(순수 검증), `lib/deal/seller-stage-persistence.ts`(RPC 호출), `lib/deal/seller-stage-actions.ts`("use server" boundary). 테스트 러너 정식화: `package.json` `test` 스크립트 + `tests/register.mjs`/`tests/resolve-alias.mjs`로 `npm test`가 `@/` alias를 해석(임시 `/tmp` loader 제거). `npm test` 222 PASS, `tsc`/`build`/`eslint` PASS. Production DB/데이터 변경 없음. 원격 migration 적용은 별도 승인 후.

2026-09-07 Phase C Live Activation — Seller Standard Stage 조회/명시적 확정 슬라이스: Seller Workspace(`/seller/deals`, S06)에 "현재 거래 단계" 패널을 추가해 (1) 현재 `deals.seller_stage_key`를 한글 Label로 표시(NULL이면 "아직 거래 단계가 확정되지 않았습니다.", 자동 DISCOVERY 금지), (2) 권한 있는 사용자(SELLER_OWNER/SELLER_OPERATOR/INTERNAL_MANAGER)가 Standard Workflow SoT(`SELLER_WORKFLOW_STAGES`)에서 단계를 선택해 명시적으로 확정하도록 했다. 모든 Seller stage write는 단일 boundary `transitionSellerDealStage`(→ RPC `transition_seller_deal_stage`, event append + 현재 stage 갱신 원자적)만 통과한다. dealId는 client가 아니라 서버 CurrentContext(active Deal)에서만 취한다. 동일 stage 재확정은 no-op(`isRedundantStageChange`)으로 중복 Event를 만들지 않는다. transition_source는 기존 convention `USER_ACTION`(명시적 사용자 행동)으로 기록하며, TOM/AI/Valuation/NDA가 stage를 자동 변경하지 않는다(자동 진행·Rigid State Machine·Mandate 필수 강제 없음). 읽기는 `readSellerDealStage`가 RLS+서버 검증 active Deal id만 사용해 타사 Deal stage 조회를 차단하고, 0018 미적용 시 컬럼 부재(42703)를 NULL로 처리해 배포 안전성을 확보한다. Progress bar/Timeline/Notification/13-step UI/Buyer/Opportunity stage는 만들지 않았다. `npm test` 223 PASS, `tsc`/`build`/`eslint` PASS. **0018 원격 적용은 이 환경에 안전한 Supabase 관리 권한(service_role/DB password/access token/CLI)이 없어 미적용 → 사용자에게 Supabase SQL Editor에서 `0018` 원문 실행 1건만 요청.** 따라서 LIVE Supabase E2E(초기 NULL→첫 확정→persistence→변경→history→same-stage no-op)와 교차회사/Buyer 차단 LIVE 확인, Production 배포 검증은 0018 원격 적용 후로 **BLOCKED(대기)**. 코드/문서는 main 병합 시 배포 안전(컬럼 부재 tolerant).

2026-09-07 Phase C Live Activation — LIVE Supabase E2E 부분 검증(진행) 및 BLOCKED 판정: 사용자가 `0018`을 Supabase Production SQL Editor에서 직접 실행("Success. No rows returned")하여 원격 적용 완료. 읽기전용 REST probe(anon, RLS 준수, service_role 미사용)로 스키마 적용 확정 — `deals.seller_stage_key` 존재(HTTP 200), `deal_stage_events` 존재(HTTP 200). **DB 보안 경계 LIVE 검증(PASS):** (a) anon `transition_seller_deal_stage` → 401 `42501 authentication required`(쓰기 차단), (b) anon `can_write_seller_deal_stage` → `false`, (c) 인증된 비참여자(신규 signup 계정) `transition_seller_deal_stage`(임의 deal_id) → 403 `42501 "not permitted to transition this deal stage"`(교차 deal/회사 쓰기 차단·deal_id 위조 불가), (d) 인증된 비참여자 `deal_stage_events` select → `[]`(RLS participant-only). **자동 검증(PASS):** `npm test` 223/223, `tsc`, `build`. **미검증(BLOCKED):** 실제 SELLER_OWNER 계정 기반 happy-path(초기 NULL 표시→첫 Stage 확정→persistence→Stage 변경→history 순서→same-stage no-op)와 seeded Seller/Buyer/타사 actor 기반 역할별 UI·RLS 확인, Production 배포 UI 확인. 원인: 이 환경에 seeded TEST 계정 로그인 수단(`VERICOM_TEST_SEED_PASSWORD`)·service_role이 없어 SELLER_OWNER 세션을 만들 수 없고, 현재 sprint에는 self-service deal 생성 경로가 없음(deal은 seed migration `0011`/`0013`로만 생성). Step 11 원칙상 service_role RLS 우회 결과를 PASS로 기록하지 않음. **슬라이스 판정 = BLOCKED**(코드/DB/보안경계는 검증됨, 역할 기반 happy-path E2E는 seeded 자격증명 1건 대기). 다음 인간 작업 1건: Secrets에 `VERICOM_TEST_SEED_PASSWORD` 추가(`scripts/sprint0-seed-auth.mjs`가 `*.sprint0@vericom.test` 계정 seed에 사용한 비밀번호).
